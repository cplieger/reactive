...
import { batch, createStore, effect, flushSync, setEffectErrorHandler, signal } from "./index.js";
...
// The drain is re-entrant by design: a write, a batch() or a flushSync() from an
// effect body sees the batch depth back at 0 and starts a NESTED drain. The pass
// counter that decides "Cycle detected" is shared by all of them, so what it
// means has to be independent of which of those reached the drain. When a nested
// drain zeroed the counter on its way out it could never reach the cap at all:
// each pass of a write cycle recurses one level and opens one nested drain from
// the body, so the recursion ran until the JS stack gave out and the consumer's
// error handler was handed a RangeError instead of the library's cycle report —
// at a depth that is a property of the runtime, not of the program.
describe("cycle bail: the threshold does not depend on how the drain was reached", () => {
  ...
  it("reports the same cycle at the same point whether or not the body calls flushSync()", () => {
    const bare = pingPong(plain);
    const flushed = pingPong(() => {
      flushSync();
    });

    expect(bare.reported).toEqual(["Cycle detected"]);
    expect(flushed.reported).toEqual(["Cycle detected"]);
    expect(flushed.reached).toBe(bare.reached);
  });
